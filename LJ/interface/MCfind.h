void Ntuplize::findSignalElectrons( const reco::GenParticle &p, std::vector<std::pair<reco::GenParticle, unsigned> > &vec, unsigned& label ) {
  //std::cerr << "starting daughter loop for a " << p.pdgId() << std::endl;
  for( reco::GenParticle::const_iterator it = p.begin(); it<p.end(); it++ ) {
    if( p.pdgId() == 25 ) { 
    	if (verbose){
    		std::cerr << std::endl;
			std::cerr << "found a Higgs!, mass:" << p.mass() << std::endl;
			std::cerr << std::endl;
    	}
...
void Ntuplize::findWDaughters( const reco::GenParticle &p, std::vector<std::pair<reco::GenParticle, unsigned> > &vec, unsigned &label ) {
  //std::cerr << "starting daughter loop for a " << p.pdgId() << std::endl;
  for( reco::GenParticle::const_iterator it = p.begin(); it<p.end(); it++ ) {
    if( abs(p.pdgId()) == 24 ) { 
    	if (verbose){
...
void Ntuplize::findZElectronsMC( const reco::GenParticle &p, std::vector<std::pair<reco::GenParticle, unsigned> > &vec, unsigned& label ) {
  //std::cerr << "starting daughter loop for a " << p.pdgId() << std::endl;
  for( reco::GenParticle::const_iterator it = p.begin(); it<p.end(); it++ ) {
    if( p.pdgId() == 23 ) { 
    	if (verbose){
...
void Ntuplize::findWElectronsMC( const reco::GenParticle &p, std::vector<std::pair<reco::GenParticle, unsigned> > &vec, unsigned& label ) {
  //std::cerr << "starting daughter loop for a " << p.pdgId() << std::endl;
  for( reco::GenParticle::const_iterator it = p.begin(); it<p.end(); it++ ) {
    if( p.pdgId() == 24 ) { 
    	if (verbose){
